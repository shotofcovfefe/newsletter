...
def _telegram_api_request(method: str, payload: ta.Dict[str, ta.Any], timeout: int = 10) -> ta.Optional[
    ta.Dict[str, ta.Any]]:
...
def send_telegram_message(chat_id: str, text: str, reply_markup: ta.Optional[ta.Dict[str, ta.Any]] = None) -> \
...
def edit_telegram_message(chat_id: str, message_id: int, text: str,
                          reply_markup: ta.Optional[ta.Dict[str, ta.Any]] = None) -> bool:
    ...
    if isinstance(resp_data, dict) and resp_data.get("description") and "message is not modified" in resp_data[
        "description"].lower():
...
def fetch_events(
    ...
    try:
        ...
        if date_from and date_to and date_from == date_to:  # Single day query
            start_of_day = isoparse(date_from).replace(tzinfo=timezone.utc).replace(hour=0, minute=0, second=0,
                                                                                    microsecond=0)
            end_of_day = start_of_day + timedelta(days=1)
        elif date_from and date_to:  # Date range query
            start_of_day = isoparse(date_from).replace(tzinfo=timezone.utc).replace(hour=0, minute=0, second=0,
                                                                                    microsecond=0)
            end_of_day = isoparse(date_to).replace(tzinfo=timezone.utc).replace(hour=0, minute=0, second=0,
                                                                                microsecond=0)
    ...
    for event in candidate_events:
        ...
        try:
            ...
            event_start_dt = isoparse(start_date_str).replace(tzinfo=timezone.utc).replace(hour=0, minute=0, second=0,
                                                                                           microsecond=0)
            # Assume end_date is inclusive, so add 1 day to make it exclusive for range checks
            event_end_dt_exclusive = isoparse(end_date_str).replace(tzinfo=timezone.utc).replace(hour=0, minute=0,
                                                                                                 second=0,
            ...
            logger.warning(
                f"Skipping event: Invalid date format ID={event.get('event_id')}, Start={start_date_str}, End={end_date_str}")
            continue
        ...
        if rule_str:
            ...
            try:
                ...
                occurrences_in_range = event_rrule.between(start_of_day, end_of_day - timedelta(microseconds=1),
                                                           inc=True)  # Make range inclusive
                ...
                logger.warning(
                    f"Skipping recurring event: Invalid rule/date ID={event.get('event_id')}, Rule='{rule_str}', Error: {e}")
        else:
    ...
    relevant_events.sort(
        key=lambda x: (x.get('_occurrence_dt', datetime.max.replace(tzinfo=timezone.utc)), x.get('card_title', '')))
...
def fetch_random_events(days_ahead: int = 7, limit: int = 1) -> ta.List[ta.Dict[str, ta.Any]]:
    ...
    try:
        ...
        data: ta.List[ta.Dict[str, ta.Any]] = resp.data if resp and hasattr(resp, 'data') and isinstance(resp.data,
                                                                                                         list) else []
        if not data: return []
        random.shuffle(data)
        return data[:limit]
        ...
        if "distance_km" in ev and postcode and user_lat is not None and user_lon is not None:
            try:
                ...
                if dist_km < 0.01:
                    ...
                    dist_str = f"{round(dist_km * 100) / 100 * 1000:.0f}m"
                elif dist_km < 10:
...
def create_event_keyboard(
    ...
    if venue_name and venue_postcode:
        try:
            search_query: str = f"{venue_name}, {venue_postcode}"; maps_url: str = f"https://www.google.com/maps/search/?api=1&query={urllib.parse.quote_plus(search_query)}"; action_row.append(
                {"text": "📍 Map", "url": maps_url})
        except Exception as e:
            logger.error(f"Error map link: {e}")
    try:
        forward_text: str = format_event_for_forwarding(event); action_row.append(
            {"text": "📤 Share", "switch_inline_query": forward_text or "Check out this event!"})
    except Exception as e:
        logger.error(f"Error formatting forward: {e}")
...
def handle_single_event_command(chat_id: str, command: str) -> None:
    ...
    command_config = {"/local": {"cb": "load_local", "ctx": "nearby", "loc": True, "days": 7},
    ...
    if is_location_based:
        ...
        if lat is None or lon is None: send_telegram_message(chat_id,
                                                             GEOCODE_ERROR_MESSAGE.format(postcode=user_pc)); return
        fetch_params.update({"user_lat": lat, "user_lon": lon});
        no_event_context = f"near {user_pc.upper()}"

    try:
        today_date = datetime.now(timezone.utc).date()
        date_from_str: ta.Optional[str] = None;
        date_to_str: ta.Optional[str] = None
        if command == "/weekend":
            today_weekday = today_date.weekday(); days_until_saturday = (
                                                                                    5 - today_weekday + 7) % 7; saturday_date = today_date + timedelta(
                days=days_until_saturday); monday_date = saturday_date + timedelta(
                days=2); date_from_str = saturday_date.strftime("%Y-%m-%d"); date_to_str = monday_date.strftime(
                "%Y-%m-%d"); no_event_context = f"starting this weekend {no_event_context}".strip()
        elif command in ["/local", "/today", "/tomorrow"]:
            days_offset = config.get("days", 0); start_date = today_date + timedelta(
                days=days_offset); end_date = start_date + timedelta(
                days=(7 if command == "/local" else 1)); date_from_str = start_date.strftime(
                "%Y-%m-%d"); date_to_str = end_date.strftime(
                "%Y-%m-%d"); no_event_context = f"starting {'in the next 7 days' if command == '/local' else time_period_context} {no_event_context}".strip()
        ...
        if command in ["/random", "/best"]:
            ...
            fetch_params["date_from"] = date_from_str;
            fetch_params["date_to"] = date_to_str
        ...
        if not event:
            base_msg = NO_EVENTS_MESSAGE.format(context=no_event_context,
        ...
        if lat is not None and lon is not None:
            try:  # Add distance
                ...
                if lat_val is not None and lon_val is not None: ev_lat = float(lat_val);ev_lon = float(lon_val);event[
                    "distance_km"] = haversine_distance(lat, lon, ev_lat, ev_lon)
            except (ValueError, TypeError) as e:
                logger.warning(f"Could not calc dist {command}: {e}")

    except Exception as e:
        logger.error(f"Error fetching event {command}: {e}", exc_info=True);
        send_telegram_message(chat_id, DEFAULT_ERROR_MESSAGE);
        return

    try:
        # Initial keyboard: No back, No forward
        keyboard = create_event_keyboard(event=event, refresh_callback_data=callback_data, can_go_back=False,
                                         can_go_forward=False, is_currently_expanded=False)
        message_text = format_events_message(events=[event], time_period=time_period_context, postcode=user_pc,
                                             user_lat=lat, user_lon=lon, show_details=False)

        sent_message_api_response = _telegram_api_request("sendMessage", {"chat_id": str(chat_id), "text": message_text,
                                                                          "parse_mode": "HTML",
                                                                          "disable_web_page_preview": True,
                                                                          "reply_markup": json.dumps(
                                                                              keyboard if keyboard else {})})

        if sent_message_api_response and sent_message_api_response.get('ok') and isinstance(
                sent_message_api_response.get("result"), dict) and (
        msg_id := sent_message_api_response["result"].get('message_id')):
            ...
            logger.error(f"Failed message_id {command}. API Response: {sent_message_api_response}")

    except Exception as e:
        logger.error(f"Error formatting/sending {command}: {e}", exc_info=True);
        send_telegram_message(chat_id, DEFAULT_ERROR_MESSAGE)
...
def handle_refresh_callback(chat_id: str, message_id: int, callback_data: str) -> None:
    ...
    if is_location_based:
        ...
        if lat is None or lon is None: edit_telegram_message(chat_id, message_id,
                                                             GEOCODE_ERROR_MESSAGE.format(postcode=user_pc),
                                                             None); return
        fetch_params.update({"user_lat": lat, "user_lon": lon});
        no_event_context = f"near {user_pc.upper()}"
    ...
    # --- ADDED: Weekend Logic ---
        ...
        time_period_context = "this weekend";
    ...
    if is_random or is_best:
        ...
        logger.error(f"Unhandled callback type in refresh handler: {callback_data}"); return
    ...
    if fetched_events:
        ...
        if potential_events:  # If filtering left any events
            ...
            logger.info(
                f"Refresh for msg {message_id}: Chose different event {event_to_show.get('event_id', 'N/A')} from {len(potential_events)} options.")
        else:  # All fetched events were the same as last shown, or only one fetched
            event_to_show = fetched_events[0]
            logger.info(
                f"Refresh for msg {message_id}: Re-showing event {event_to_show.get('event_id', 'N/A')} or only one option found.")
    ...
    if event_to_show:
        ...
        user_pc_refresh, lat_refresh, lon_refresh = (user_pc, lat, lon) if is_location_based else get_user_location(
            chat_id)
        ...
        if lat_refresh is not None and lon_refresh is not None:
            try:
                ...
                if not math.isnan(ev_lat) and not math.isnan(ev_lon): event_to_show["distance_km"] = haversine_distance(
                    lat_refresh, lon_refresh, ev_lat, ev_lon)
            except:
                pass
        # Update History
        ...
        if history_key not in message_context_type: message_context_type[
            history_key] = callback_data  # Store original context
        ...
        keyboard = create_event_keyboard(event_to_show, callback_data, can_go_back,
                                         is_currently_expanded=False)  # Reset to collapsed view
        ...
        final_no_event_msg = NO_EVENTS_MESSAGE.format(context=f"further {no_event_context}",
                                                      postcode=user_pc.upper() if user_pc else "your area")
...
def send_event_messages(
    ...
    for event in events:
        ...
        if message:
            ...
            logger.warning(
                f"Empty message generated by format_events_message for event: {event.get('event_id', 'N/A')}")
...
def process_message(msg: ta.Dict[str, ta.Any]) -> None:
    ...
    if text_lower == "/subscribe":
        if chat_type == 'private':
            ...
            try:  # Keep original try/except around DB operation
                supabase.table("telegram_subscribers").upsert(
                    {"chat_id": str(chat_id), "subscribed_date": datetime.now(timezone.utc).isoformat()},
                    on_conflict="chat_id").execute()
    ...
    # --- Postcode Handling ---
        ...
        try:
            pc_lat, pc_lon = geocode_postcode_to_latlon(postcode_norm)
        except Exception as e:
            logger.error(f"Geocoding error for '{postcode_norm}': {e}")
        ...
        if is_updating:
            if pc_lat is not None and pc_lon is not None:
                ...
                if set_user_postcode(chat_id, postcode_norm):
                    ...
                    send_telegram_message(chat_id, "⚠️ There was an error saving your postcode.")
            else:
                send_telegram_message(chat_id,
                                      f"⚠️ Couldn't validate postcode {postcode_norm}. Please try a different London postcode.")
            awaiting_location_update[chat_id] = False
            ...
            if pc_lat is not None and pc_lon is not None:
                ...
                if events_pc:
                    ...
                    send_event_messages(chat_id=chat_id, events=events_pc, postcode=postcode_norm, user_lat=pc_lat,
                                        user_lon=pc_lon)
                else:
                    # Updated context string to mention "starting"
                    send_telegram_message(chat_id,
                                          f"Couldn't find any events starting near {postcode_norm} in the next 7 days.")
            else:
                send_telegram_message(chat_id, f"Sorry, I couldn’t find coordinates for {postcode_norm}.")
        return
...
def process_callback_query(callback_query: ta.Dict[str, ta.Any]) -> None:
    ...
    if data.startswith("load_"):
        ...
        try:
            ...
            lon: ta.Optional[float] = None
            fetch_limit = 5;
            is_location_based = data in ["load_local", "load_today", "load_tomorrow", "load_weekend"];
            is_random = data == "load_random";
            is_best = data == "load_best";
            time_period_context: str = "";
            no_event_context: str = ""
            event_to_show: ta.Optional[ta.Dict[str, ta.Any]] = None;
            fetched_events: ta.List[ta.Dict[str, ta.Any]] = []
            if is_location_based: user_pc, lat, lon = get_user_location(
                chat_id);  # Error handling inside function call below
            date_from_str: ta.Optional[str] = None;
            date_to_str: ta.Optional[str] = None;
            today_date = datetime.now(timezone.utc).date()
            if data == "load_local":
                date_from_str = today_date.strftime("%Y-%m-%d"); date_to_str = (
                            today_date + timedelta(days=7)).strftime(
                    "%Y-%m-%d"); time_period_context = "nearby"; no_event_context = f"starting in the next 7 days {no_event_context}".strip()
            elif data == "load_today":
                date_from_str = today_date.strftime("%Y-%m-%d"); date_to_str = (
                            today_date + timedelta(days=1)).strftime(
                    "%Y-%m-%d"); time_period_context = "today"; no_event_context = f"starting today {no_event_context}".strip()
            elif data == "load_tomorrow":
                tomorrow_date = today_date + timedelta(days=1); date_from_str = tomorrow_date.strftime(
                    "%Y-%m-%d"); date_to_str = (tomorrow_date + timedelta(days=1)).strftime(
                    "%Y-%m-%d"); time_period_context = "tomorrow"; no_event_context = f"starting tomorrow {no_event_context}".strip()
            elif data == "load_weekend":
                today_weekday = today_date.weekday(); days_until_saturday = (
                                                                                        5 - today_weekday + 7) % 7; saturday_date = today_date + timedelta(
                    days=days_until_saturday); monday_date = saturday_date + timedelta(
                    days=2); date_from_str = saturday_date.strftime("%Y-%m-%d"); date_to_str = monday_date.strftime(
                    "%Y-%m-%d"); time_period_context = "this weekend"; no_event_context = f"starting this weekend {no_event_context}".strip()
            if is_random or is_best:
                fetched_events = fetch_random_events(days_ahead=DEFAULT_RANDOM_DAYS_AHEAD,
                                                     limit=fetch_limit); time_period_context = "a random event" if is_random else "a top pick"; no_event_context = "randomly" if is_random else "as a top pick"
            elif is_location_based or data in ["load_local", "load_today", "load_tomorrow", "load_weekend"]:
                fetch_params["date_from"] = date_from_str; fetch_params["date_to"] = date_to_str; fetch_params[
                    "user_lat"] = lat; fetch_params["user_lon"] = lon; fetched_events = fetch_events(
                    overall_limit=fetch_limit, **fetch_params)
            else:
                logger.error(f"Unhandled cb type {data}"); return
            current_event_id = current_event.get('event_id') if current_event else None
            if fetched_events: potential_events = [e for e in fetched_events if e.get(
                'event_id') != current_event_id]; event_to_show = random.choice(
                potential_events) if potential_events else fetched_events[0]
            logger.info(f"Refresh selected {event_to_show.get('event_id') if event_to_show else 'None'}")

            if event_to_show:
                user_pc_refresh, lat_refresh, lon_refresh = (
                user_pc, lat, lon) if is_location_based else get_user_location(chat_id)
                if lat_refresh is not None and lon_refresh is not None:
                    try:  # Add distance
                        ...
                        logger.warning(f"Failed dist refresh: {e}")

                # Clear any forward cache from previous back action
                message_forward_cache.pop(history_key, None)

                # Append NEW event to history
                if history_key not in message_event_history: message_event_history[history_key] = deque(
                    maxlen=HISTORY_SIZE)
                message_event_history[history_key].append(event_to_show.copy())
                ...
                keyboard = create_event_keyboard(event=event_to_show, refresh_callback_data=data, can_go_back=True,
                                                 can_go_forward=False, is_currently_expanded=current_state_expanded)
                message_text = format_events_message(events=[event_to_show], postcode=user_pc_refresh,
                                                     user_lat=lat_refresh, user_lon=lon_refresh,
                                                     time_period=time_period_context,
                                                     show_details=current_state_expanded)
                if not edit_telegram_message(chat_id, message_id, message_text, reply_markup=keyboard): logger.error(
                    f"Edit failed refresh {data} msg {message_id}")
            else:
                final_no_event_msg = NO_EVENTS_MESSAGE.format(context=f"further events {no_event_context}",
                                                              postcode=user_pc.upper() if user_pc else "your area")
                edit_telegram_message(chat_id, message_id, final_no_event_msg, reply_markup=None)
        except Exception as e:
            logger.error(f"Error handling refresh callback {data}: {e}", exc_info=True)
            try:
                edit_telegram_message(chat_id, message_id, DEFAULT_ERROR_MESSAGE, None)
            except Exception:
                pass
    ...
    # --- Back Action ---
        ...
        if history and len(history) > 1:
            try:
                ...
                logger.error(f"History empty after pop msg {message_id}"); edit_telegram_message(chat_id, message_id,
            ...
            if current_event and original_context:
                try:
                    ...
                    keyboard = create_event_keyboard(current_event, original_context, can_go_back=False,
                                                     ...
                                                     is_currently_expanded=current_state_expanded)  # No forward if already at start
    ...
    # --- Forward Action ---
        ...
        if event_to_show and original_context:  # Can only go forward if cache existed
            try:
                ...
                message_event_history[history_key].append(event_to_show.copy())
                logger.info(
                    f"Appended history msg {message_id} via show_next. New len: {len(message_event_history[history_key])}")
                ...
                if lat_fwd is not None and lon_fwd is not None:
                    try:  # Add distance
                        ...
                        lon_val = event_to_show.get("longitude")
                        if lat_val is not None and lon_val is not None: ev_lat = float(lat_val);ev_lon = float(lon_val);
                        event_to_show["distance_km"] = haversine_distance(lat_fwd, lon_fwd, ev_lat, ev_lon)
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Failed dist fwd: {e}")

                current_state_expanded = message_expansion_state.get(history_key, False)
                keyboard = create_event_keyboard(event=event_to_show, refresh_callback_data=original_context,
                                                 can_go_back=can_go_back, can_go_forward=can_go_forward,
                                                 is_currently_expanded=current_state_expanded)
                message_text = format_events_message(events=[event_to_show], postcode=user_pc_fwd, user_lat=lat_fwd,
                                                     user_lon=lon_fwd, show_details=current_state_expanded)
                if not edit_telegram_message(chat_id, message_id, message_text, reply_markup=keyboard): logger.error(
                    f"Edit failed show_next msg {message_id}")

            except Exception as e:
                logger.error(f"Error show_next: {e}", exc_info=True); edit_telegram_message(chat_id, message_id,
            ...
            if current_event and original_context:
                try:
                    ...
                    current_state_expanded = message_expansion_state.get(history_key, False)
                    keyboard = create_event_keyboard(current_event, original_context, can_go_back=can_go_back_state,
                                                     can_go_forward=False, is_currently_expanded=current_state_expanded)
                    _telegram_api_request("editMessageReplyMarkup", {"chat_id": chat_id, "message_id": message_id,
                                                                     "reply_markup": json.dumps(
                                                                         keyboard if keyboard else {})})
                except Exception as e:
                    logger.error(f"Error editing keyboard show_next (at end): {e}")
            ...
            try:
                ...
                if lat_toggle is not None and lon_toggle is not None and "distance_km" not in event_to_show:
                    try:  # Add distance
                        ...
                        lon_val = event_to_show.get("longitude")
                        if lat_val is not None and lon_val is not None: ev_lat = float(lat_val);ev_lon = float(lon_val);
                        event_to_show["distance_km"] = haversine_distance(lat_toggle, lon_toggle, ev_lat, ev_lon)
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Failed dist toggle: {e}")

                message_text = format_events_message(events=[event_to_show], postcode=user_pc_toggle,
                                                     user_lat=lat_toggle, user_lon=lon_toggle,
                                                     show_details=show_details_new_state)
                keyboard = create_event_keyboard(event=event_to_show, refresh_callback_data=original_context,
                                                 can_go_back=can_go_back_toggle, can_go_forward=can_go_forward_toggle,
                                                 is_currently_expanded=show_details_new_state)
                ...
                if edit_telegram_message(chat_id, message_id, message_text, reply_markup=keyboard):
                    ...
                    logger.error(f"Edit failed {data} msg {message_id}")
            except IndexError:
                logger.error(f"History empty toggle msg {message_id}"); edit_telegram_message(chat_id, message_id,
                                                                                              DEFAULT_ERROR_MESSAGE,
                                                                                              None)
            except Exception as e:
                logger.error(f"Error {data}: {e}", exc_info=True); edit_telegram_message(chat_id, message_id,
                                                                                         DEFAULT_ERROR_MESSAGE, None)
        else:
            logger.warning(f"History/context not found {data} msg {message_id}")
...
def broadcast_newsletter(n_events: int = DEFAULT_BROADCAST_LIMIT) -> None:
    ...
    try:
        ...
        logger.error(f"Error fetching subscribers for broadcast: {exc}", exc_info=True);
        return
    ...
    for sub in subscribers:
        ...
        try:
            ...
            if user_pc and lat is not None and lon is not None:
                # Calls updated fetch_events
                events_to_send = fetch_events(date_from=today_str, date_to=future_str, user_lat=lat, user_lon=lon,
                                              overall_limit=n_events)
    ...
    logger.info(
        f"Broadcast finished. Sent to {sent_count}, Failed/No Events for {failed_count}/{len(subscribers)} subscribers.")
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"Critical error fetching updates: {e}", exc_info=True); time.sleep(15); continue
        ...
        for upd in updates:
            try:
                ...
                if message and (chat_id := message.get('chat', {}).get('id')):
                    try:
                        send_telegram_message(str(chat_id), DEFAULT_ERROR_MESSAGE)
                    except Exception:
                        pass  # Ignore failure to notify
